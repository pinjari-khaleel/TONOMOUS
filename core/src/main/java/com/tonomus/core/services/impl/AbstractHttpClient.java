...
public abstract class AbstractHttpClient {
  ...
  protected void addPayload(final @NonNull HttpEntityEnclosingRequestBase base,
    ...
    log.debug("POST payload: {}", StringUtils.join(payload));
  ...
  protected <T> EndpointResponse<T> executeRequest(Type type, HttpRequestBase httpRequestBase) {
    ...
    log.info("Starting stopwatch for request: {}", httpRequestBase.getURI().toString());
    stopWatch.start();
    ...
    try (CloseableHttpClient httpClient = getHttpClient()) {
      ...
      stopWatch.stop();
      log.info("*** Stopwatch time: {}", stopWatch);
      stopWatch.reset();
    }
  }
        ...
        .addInterceptorLast(new HttpResponseInterceptor() {
          ...
          public void process(HttpResponse response, HttpContext context) throws IOException {
            if (response.getStatusLine().getStatusCode() == INTERNAL_SERVER_ERROR_STATUS) {
              throw new IOException("Retry http request after response status 500");
            }
          }
        }).build();
  }
