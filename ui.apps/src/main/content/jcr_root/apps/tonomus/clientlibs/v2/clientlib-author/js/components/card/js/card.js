...
(function($, Granite) {
  ...
  var dialogContentSelector = ".cmp-card__editor";
  var actionsMultifieldSelector = ".cmp-card__editor-multifield_actions";
  var titleCheckboxSelector = 'coral-checkbox[name="./titleFromPage"]';
  var titleTextfieldSelector = '.cq-RichText-editable[name="./jcr:title"]';
  ...
  $(document).on("dialog-loaded", function(e) {
      ...
      if (dialogContent) {
          ...
          var $titleTextfield = $(titleTextfieldSelector);
          if ($titleTextfield.length) {

              var rteInstance = $titleTextfield.data("rteinstance");
              // wait for the description textfield rich text editor to signal start before initializing.
              // Ensures that any state adjustments made here will not be overridden.
              if (rteInstance && rteInstance.isActive) {
                  init(e, $dialog, $dialogContent, dialogContent);
              } else {
                  $titleTextfield.on("editing-start", function() {
                      init(e, $dialog, $dialogContent, dialogContent);
                  });
              } 
          } else {
  ...
  function init(e, $dialog, $dialogContent, dialogContent) {
      titleTuple = new CheckboxTextfieldTuple(dialogContent, titleCheckboxSelector, titleTextfieldSelector, true);
      ...
      $actionsMultifield.on("change", function(event) {
          ...
          if ($target.is("foundation-autocomplete")) {
              ...
              if (event.target.items.length === 1 && $first.is("coral-multifield-item")) {
                  var $input = $first.find(".cmp-card__editor-actionField-linkUrl");
  ...
  function retrievePageInfo(dialogContent) {
      ...
      if (linkURL === undefined || linkURL === "") {
          url = dialogContent.find('.cmp-card__editor-multifield_actions [data-cmp-card-v2-dialog-edit-hook="actionLink"]').val();
      } else {
          url = linkURL;
      }
  ...
  function updateText(target) {
      var url = target.val();
      if (url && url.startsWith("/")) {
          var textField = target.parents("coral-multifield-item").find('[data-cmp-card-v2-dialog-edit-hook="actionTitle"]');
