...
const argv = require('yargs')
  ...
  .command(
    'critical-css',
    'Build with pages being optimized with critical-css',
    () => {},
    buildAllCriticalCss,
  )
...
function buildAll() {
  ...
  buildCode()
    .then(() => buildHTML({ cleanPartials: true, criticalCss: false }))
    .then(handleBuildComplete);
}

function buildAllCriticalCss() {
  cleanDist();

  buildCode()
    .then(() => buildHTML({ cleanPartials: true, criticalCss: true }))
    .then(handleBuildComplete);
}
