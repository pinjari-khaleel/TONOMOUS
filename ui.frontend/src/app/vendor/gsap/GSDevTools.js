...
_gsScope._gsDefine(
  ...
  function () {
    var _doc = document,
      ...
      _getClippedDuration = function (animation, excludeRootRepeats) {
        ...
        while (t) {
          max = Math.max(
            ...
            t.totalDuration() > 999
              ? t.endTime(false)
              : t._startTime + t._totalDuration / t._timeScale,
          );
          t = t._next;
        }
        return !excludeRootRepeats && repeat
          ? max * (repeat + 1) + animation._repeatDelay * repeat
          : max;
      },
      ...
      _selectValue = function (element, value, label, insertIfAbsent) {
        ...
        if (insertIfAbsent) {
          ...
          option.innerHTML = label.innerHTML =
            typeof insertIfAbsent === 'string' ? insertIfAbsent : value;
      ...
      _recordedRoot = new TimelineLite({
        data: 'root',
        id: 'Global Timeline',
        autoRemoveChildren: false,
        smoothChildTiming: true,
      }),
      _recordedTemp = new TimelineLite({
        data: 'root',
        id: 'Global Temp',
        autoRemoveChildren: false,
        smoothChildTiming: true,
      }),
      _rootTween = TweenLite.to(_recordedRoot, 1, {
        time: 1,
            ...
            while (t) {
              ...
              if (
                !(
                  ...
                  t.target === t.vars.onComplete &&
                  !t._duration
                ) &&
                !(t.target && t.target._gsIgnore)
              ) {
      ...
      _buildPlayPauseMorph = function (svg) {
        ...
        tl.to(svg.querySelector('.play-1'), 0.5, {
          attr: {
            d:
      ...
      _buildLoopAnimation = function (svg) {
        ...
        tl.to(svg, 0.5, { rotation: 360, ease: Power3.easeInOut, transformOrigin: '50% 50%' }).to(
          svg.querySelectorAll('.loop-path'),
          0.5,
          { fill: '#91e600', ease: Linear.easeNone },
          0,
        );
        return tl;
      },
      GSDevTools = function (vars) {
        ...
        var _self = this,
          ...
          animation = function (anim) {
            ...
            startTime = 0;
            maxDuration =
              vars.maxDuration || Math.min(1000, _getClippedDuration(selectedAnimation));
    ...
    TimelineLite.prototype._remove = function (tween, skipDisable) {
    ...
    TweenLite.onOverwrite = function (
      ...
      if (GSDevTools.logOverwrites) {
        if (overwrittenProperties) {
          console.log(
            '[Overwrite warning] the following properties were overwritten: ',
            overwrittenProperties,
            '| target:',
            target,
            '| overwritten tween: ',
          ...
          console.log(
            '[Overwrite warning] the following tween was overwritten:',
            overwrittenTween,
            'by',
            overwritingTween,
          );
