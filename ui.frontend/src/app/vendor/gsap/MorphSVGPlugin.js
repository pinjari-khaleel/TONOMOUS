...
var _PI = Math.PI,
    ...
    for (i = 0; i < elements; i++) {
      ...
      if (command === 'M') {
        ...
        beziers = _arcToBeziers(
          ...
          +a[i + 4],
          +a[i + 5],
          (isRelative ? relativeX : 0) + a[i + 6] * 1,
          (isRelative ? relativeY : 0) + a[i + 7] * 1,
        );
        if (beziers) {
  ...
  _getTotalSize = function (rawPath, samplesPerBezier) {
  ...
  _getClosestAnchor = function (rawPath, x, y) {
  ...
  _equalizeSegmentQuantity = function (start, end, shapeIndex, map, fillSafe) {
...
MorphSVGPlugin.convertToPath = function (targets, swap) {
  ...
  var a =
      !targets || targets.length === 0
        ? []
        : targets.length && targets[0] && targets[0].nodeType
